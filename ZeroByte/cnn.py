...
def combined_loss(pred_class_scores, true_classes, pred_bboxes, true_bboxes):
    class_loss = F.cross_entropy(pred_class_scores, true_classes)
...
image_folder = r'C:\Users\Ahmad\PycharmProjects\ZeroByte\images'
label_folder = r'C:\Users\Ahmad\PycharmProjects\ZeroByte\labels'
dataset = FoodWasteDataset(image_folder=image_folder, label_folder=label_folder, transform=transform)
data_loader = DataLoader(dataset, batch_size=8, shuffle=True, collate_fn=lambda batch: tuple(zip(*batch)))
...
unlabeled_folder = r'C:\Users\Ahmad\PycharmProjects\ZeroByte\train'
output_label_folder = r'C:\Users\Ahmad\PycharmProjects\ZeroByte\new food waste annotations'
...
with torch.no_grad():
    for img_file in os.listdir(unlabeled_folder):
        ...
        print(f"\nProcessing {img_file}")
        print("Predicted class scores:", pred_class_scores)
        ...
        for idx in range(pred_class_scores.size(0)):
            ...
            if idx < pred_bboxes.size(0):
                ...
                print(f"Class: {pred_class.item()}, Confidence: {confidence.item()}, Bounding Box: {pred_bbox}")
